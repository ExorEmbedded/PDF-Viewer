    ...
    if (m_zoom == 0)
    {
     //Fit to size
		...
		if(m_rotation == 0 || m_rotation == 180)
			pWidth = popplerPage->pageSize().width();
		else
			pWidth = popplerPage->pageSize().height();

		zoom = (((((QWidget*)parent())->width() * 1.0)/m_dpiX) * 72)/(pWidth+1);
	} else
		zoom = m_zoom;

	const double resX = m_dpiX * zoom;
	const double resY = m_dpiY * zoom;
