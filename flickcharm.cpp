...
/*
void FlickCharm::activateSubWidgetOn(QTreeWidgetItem *subWidget, QWidget *widget)
{
    QWidget* parentWidget = NULL;
    QAbstractScrollArea *scrollArea = qobject_cast<QAbstractScrollArea*>(widget);
    if (scrollArea) {
...
{
	const int deltaSpeed = deltatime*2.5;
...
bool FlickCharm::eventFilter(QObject *object, QEvent *event)
{
	...
	FlickData *data = d->flickData.value(viewport, NULL);
	if (data == NULL) {
		// search subwidgets
